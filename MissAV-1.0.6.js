            ...
            params: [
                {
                    ...
                    description: "选择分类",
                    enumOptions: [
                        { title: "无码流出", value: "https://missav.fans/dm817/cn/uncensored-leak" },
                        { title: "FC2", value: "https://missav.fans/dm597/cn/fc2" },
                        { title: "HEYZO", value: "https://missav.fans/dm2208642/cn/heyzo" },
                        { title: "东京热", value: "https://missav.fans/dm42/cn/tokyohot" },
                        { title: "Caribbeancom", value: "https://missav.fans/dm7704788/cn/caribbeancom" },
                        { title: "Gachinco", value: "https://missav.fans/dm150/cn/gachinco" },
                        { title: "XXX-AV", value: "https://missav.fans/dm42/cn/xxxav" },
                        { title: "人妻斩", value: "https://missav.fans/dm37/cn/marriedslash" },
                        { title: "顽皮 4610", value: "https://missav.fans/dm33/cn/naughty4610" },
                        { title: "顽皮 0930", value: "https://missav.fans/dm37/cn/naughty0930" }
                    ]
                },
                {
                    name: "sort_by",
            ...
            params: [
                {
                    ...
                    description: "选择分类",
                    enumOptions: [
                        { title: "麻豆传媒", value: "https://missav.fans/dm63/cn/madou" },
...
async function loadTodayHot(params = {}) {
    ...
    let url = "https://missav.fans/dm301/cn/today-hot?sort=today_views";
...
async function loadWeeklyHot(params = {}) {
    ...
    let url = "https://missav.fans/dm170/cn/weekly-hot?sort=weekly_views";
...
async function loadMonthlyHot(params = {}) {
    ...
    let url = "https://missav.fans/dm273/cn/monthly-hot?sort=monthly_views";
...
async function loadNewRelease(params = {}) {
    ...
    let url = "https://missav.fans/dm635/cn/release?sort=released_at";
...
async function loadChineseSubtitle(params = {}) {
    ...
    let url = "https://missav.fans/dm278/cn/chinese-subtitle";
    let hasParams = false;
...
function parseVideoList(html) {
    ...
    $('a[href]').each((index, element) => {
        ...
        if ($img.length && href.match(/\/cn\/[a-zA-Z0-9\-]+(-uncensored-leak)?$/)) {
            const slugMatch = href.match(/\/cn\/([a-zA-Z0-9\-]+(?:-uncensored-leak)?)$/);
            const slug = slugMatch ? slugMatch[1].toLowerCase() : "";
            const blockedSlugs = {
                vip: 1, actresses: 1, genres: 1, makers: 1, saved: 1,
                playlists: 1, history: 1, new: 1, vr: 1, "actresses/ranking": 1
            };
            if (!slug || blockedSlugs[slug]) {
                return;
            }
...
// Media playlist path preference. Master playlist.m3u8 often plays muted
// in system players (separate audio not attached). Never rewrite a real
// variant onto a path that does not exist for that title.
const MEDIA_PATH_PREF = [
    "720p/video.m3u8",
    "1280x720/video.m3u8",
    "1080p/video.m3u8",
...
function isMediaPlaylistUrl(url) {
    return /\/(?:\d+p|\d+x\d+)\/video\.m3u8(\?.*)?$/i.test(url || "");
}
...
function cdnBaseFromUuid(uuid) {
    return uuid ? `https://surrit.mrstcdn.store/${uuid}` : "";
}

function joinVariantUrl(baseUrl, variantPath) {
    if (!variantPath) return "";
    if (/^https?:\/\//i.test(variantPath)) return variantPath;
    const base = (baseUrl || "").replace(/\/playlist\.m3u8(\?.*)?$/i, "").replace(/\/$/, "");
    return `${base}/${variantPath.replace(/^\//, "")}`;
}

function pickPreferredMediaUrl(candidateUrls) {
    const urls = (candidateUrls || []).map(cleanM3u8Url).filter(Boolean);
    if (urls.length === 0) return "";
    for (const pref of MEDIA_PATH_PREF) {
        const hit = urls.find((u) => u.toLowerCase().includes("/" + pref.toLowerCase()));
        if (hit) return hit;
    }
    const media = urls.find(isMediaPlaylistUrl);
...
function parseMasterVariants(masterText, masterUrl) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        variants.push({
            url: joinVariantUrl(masterUrl, next),
            resolution: (line.match(/RESOLUTION=(\d+x\d+)/i) || [])[1] || "",
            codecs: codecsMatch ? codecsMatch[1] : "",
            hasAac: codecsMatch ? /mp4a/i.test(codecsMatch[1]) : false
        });
    }
    return variants;
}
...
function unpackDeanEdwards(html) {
    if (!html) return "";
    // Packed payload uses escaped quotes: }('e=\'https://...\';',15,15,'m3u8|uuid|...'.split('|'),0,{})
    const re = /eval\(function\(p,a,c,k,e,d\)[\s\S]{0,900}\('((?:\\'|[^'])*)',(\d+),(\d+),'([^']+)'\.split\('\|'\)/g;
    let match;
    let best = "";
    while ((match = re.exec(html)) !== null) {
        ...
        if (dict.indexOf("playlist") === -1 && dict.indexOf("m3u8") === -1 && dict.indexOf("surrit") === -1) {
            continue;
        }
        ...
        for (let i = 0; i < dict.length; i++) {
            if (dict[i]) map[encode(i)] = dict[i];
        }
        const decoded = template.replace(/\b[0-9a-z]+\b/g, (tok) => map[tok] || tok);
        if (decoded.indexOf("http") !== -1) best = decoded;
    }
    return best;
}

function extractPackedSources(html) {
    const decoded = unpackDeanEdwards(html);
    const sources = { decoded, source: "", source842: "", source1280: "" };
    if (!decoded) return sources;
    const assignRe = /([A-Za-z_][A-Za-z0-9_]*)\s*=\s*'(https?:\/\/[^']+)'/g;
    let m;
    while ((m = assignRe.exec(decoded)) !== null) {
        const name = m[1];
        const url = cleanM3u8Url(m[2]);
        if (name === "source" || name === "source842" || name === "source1280") {
            sources[name] = url;
        }
    }
    return sources;
...
function collectCandidateM3u8(html) {
    const urls = [];
    const packed = extractPackedSources(html);
    [packed.source1280, packed.source842, packed.source].forEach((u) => {
        if (u) urls.push(u);
    });

    const plain = html.match(/https?:\/\/[^"'\\\s<>]*surrit[^"'\\\s<>]*\/[a-f0-9\-]+\/[^"'\\\s<>]*\.m3u8/gi) || [];
    plain.forEach((u) => urls.push(cleanM3u8Url(u)));

    const uuid = extractUuidFromText(html);
    if (uuid) {
        const base = cdnBaseFromUuid(uuid);
        urls.push(`${base}/playlist.m3u8`);
        MEDIA_PATH_PREF.forEach((p) => urls.push(`${base}/${p}`));
    }
...
function extractM3u8FromHtml(html) {
    if (!html) return "";
    const packed = extractPackedSources(html);
    const preferred = pickPreferredMediaUrl([packed.source1280, packed.source842]);
    if (preferred && isMediaPlaylistUrl(preferred)) return preferred;

    const candidates = collectCandidateM3u8(html).filter(isMediaPlaylistUrl);
    if (candidates.length > 0) return pickPreferredMediaUrl(candidates);

    if (packed.source && isMediaPlaylistUrl(packed.source)) return packed.source;
    return preferred || packed.source || "";
}

function extractMasterUrlFromHtml(html) {
    const packed = extractPackedSources(html);
    if (packed.source && isMasterPlaylistUrl(packed.source)) return packed.source;
    const uuid = extractUuidFromText(html);
    return uuid ? `${cdnBaseFromUuid(uuid)}/playlist.m3u8` : "";
}
...
async function resolvePlayableM3u8(html, pageLink) {
    const headers = buildStreamHeaders(pageLink);
    const packed = extractPackedSources(html);
    const masterUrl = extractMasterUrlFromHtml(html);
    let variants = [];

    if (masterUrl) {
        const masterText = await fetchText(masterUrl, headers);
        variants = parseMasterVariants(masterText, masterUrl);
    }

    const fromMaster = variants.map((v) => v.url);
    const packedMedia = [packed.source1280, packed.source842].filter(isMediaPlaylistUrl);
    const htmlMedia = collectCandidateM3u8(html).filter(isMediaPlaylistUrl);

    let playUrl = pickPreferredMediaUrl(fromMaster.length ? fromMaster : packedMedia.concat(htmlMedia));

    if (playUrl) {
        const body = await fetchText(playUrl, headers);
        if (body && body.indexOf("#EXTM3U") === 0) {
            return playUrl;
        }
        const fallbackPool = fromMaster.concat(packedMedia, htmlMedia);
        for (let i = 0; i < fallbackPool.length; i++) {
            const cand = fallbackPool[i];
            if (!cand || cand === playUrl) continue;
            const t = await fetchText(cand, headers);
            if (t && t.indexOf("#EXTM3U") === 0 && isMediaPlaylistUrl(cand)) {
                return cand;
            }
        }
        if (isMediaPlaylistUrl(playUrl)) return playUrl;
    }
...
function buildDetailResult(link, videoId, videoCode, title, videoUrl) {
    const headers = buildStreamHeaders(link);
    const useAppPlayer = !!videoUrl && /\.m3u8(\?|$)/i.test(videoUrl);
    return {
        id: link,
        type: "detail",
...
async function loadDetail(link) {
    ...
    try {
        ...
        if (!response || !response.data || response.data.includes('Just a moment') || response.data.length < 50000) {
            return buildDetailResult(link, videoId, videoCode, videoCode, "");
        }
        ...
        const videoUrl = await resolvePlayableM3u8(response.data, link);
        return buildDetailResult(link, videoId, videoCode, title, videoUrl);
